...
| pkColumn / lockColumn | update/delete WHERE 기준 PK, 낙관적 잠금 컬럼 |
...
3. ScaffoldService      : 산출물 생성 -> 탭으로 복사용 코드 표시
...
파일 적용은 `local` profile에서만 가능하다. `moduleName`, `domainId`, `domainClass`는 저장 경로 오염을 막기 위해 각각 패턴 검증을 통과해야 한다.
...
- CRUD 모드는 선택한 `pkColumn`을 update/delete WHERE와 delete 파라미터 타입에 반영하고, 선택한 `lockColumn`을 낙관적 잠금 조건으로 사용한다.
- CRUD 모드의 수정 요청은 **화이트리스트 원칙을 유지한다.** `*UpdateRequestDTO`는 `pkColumn`, `before{LockColumn}`, `editable=true` 컬럼만 선언한다. VO 전체 컬럼이나 모달에 표시된 전체 컬럼을 update 요청 DTO로 사용하지 않는다.
...
- CRUD Mapper XML은 `targetTable`, editable 컬럼, PK, lock 컬럼 기준으로 `INSERT/UPDATE/DELETE`를 생성한다.
