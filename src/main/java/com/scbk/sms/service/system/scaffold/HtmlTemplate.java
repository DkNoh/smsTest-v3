...
public final class HtmlTemplate {
    ...
    public static String generate(ScaffoldModel model) {
        ...
        sb.append("                <div class=\"col-auto ms-auto d-flex gap-2\">\n")
          ...
          .append("                    <button type=\"button\" id=\"btn-search\" class=\"btn btn-primary px-4\"><i data-lucide=\"search\"></i><span>조회</span></button>\n")
          .append("                    <button type=\"button\" id=\"btn-reset\" class=\"btn btn-secondary px-3\"><i data-lucide=\"rotate-ccw\"></i><span>초기화</span></button>\n");
        if (model.includeExcel()) {
            sb.append("                    <button type=\"button\" id=\"btn-excel\" th:if=\"${pageAuth.download}\" class=\"btn btn-success ms-1\"><i data-lucide=\"download\"></i><span>엑셀</span></button>\n");
        }
        sb.append("                </div>\n")
          .append("            </div>\n")
          .append("        </div>\n")
    ...
    private static void appendInput(StringBuilder sb, ScaffoldModel.SearchParam param) {
        if (param.isSelect()) {
            sb.append("<select id=\"").append(param.name()).append("\" class=\"form-select scaffold-search-control\">\n")
        ...
        if (param.isRadio()) {
            sb.append("<div id=\"").append(param.name()).append("\" class=\"d-flex align-items-center gap-2 scaffold-radio-group\">\n")
            ...
            for (Option option : parseOptions(param.optionsText())) {
                sb.append("                        <label class=\"form-check-label\"><input class=\"form-check-input me-1\" type=\"radio\" name=\"")
        ...
        sb.append("<input type=\"text\" id=\"").append(param.name())
          .append("\" class=\"form-control scaffold-search-control\">");
    }
    ...
    private static String createButton(ScaffoldModel model) {
        ...
        return "                    <button type=\"button\" id=\"btn-create\" th:if=\"${pageAuth.create}\" class=\"btn btn-outline-primary px-3\"><i data-lucide=\"plus\"></i><span>등록</span></button>\n";
    }
    ...
    private static void appendDatePickerInput(StringBuilder sb, String inputId, int width) {
        sb.append("<div class=\"scaffold-date-field ").append(dateFieldSizeClass(width)).append("\">")
          ...
          .append("<span class=\"tui-ico-date\"></span>")
          .append("</div>")
